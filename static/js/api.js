...
const API = {
    ...
    async sendEmailWithSendGridTemplate(fromEmail, toEmail, subject, sendgridTemplateId, dynamicData, campaignTemplateId) {
        // The key fix here is explicitly passing sendgrid_template_id and dynamic_template_data
        // while ensuring the 'body' field is omitted from the payload (by not including it
        // in the JSON.stringify object) to prevent conflicts with the SendGrid template logic 
        // implemented in main.py.

        // Note: 'template_id' here refers to the local database Campaign Template ID
        // used for logging/lookup in the backend.
        ...
        return await API.fetch('/api/send-email', {
            ...
            body: JSON.stringify({
                ...
                to_email: toEmail,
                subject: subject, // Send subject, even if template overrides it (schema requirement)
                sendgrid_template_id: sendgridTemplateId,
                dynamic_template_data: dynamicData,
                template_id: campaignTemplateId // Local DB ID for tracking
                // IMPORTANT: We explicitly omit the 'body' field here.
            })
        });
    },
    ...
    async sendEmailWithTemplate(fromEmail, toEmail, subject, body, campaignTemplateId) {
        // This function handles the standard HTML template case, where 'body' is required.
        return await API.fetch('/api/send-email', {
            method: 'POST',
            body: JSON.stringify({
                from_email: fromEmail,
                to_email: toEmail,
                subject: subject,
                body: body, // Include body only for custom HTML emails
                template_id: campaignTemplateId // Local DB ID for template lookup
            })
        });
    },
    ...
    async requestTemplate(template) {
        return await API.fetch('/templates/request', {
            method: 'POST',
            body: JSON.stringify(template)
        });
    },
    ...
    async getPendingTemplates() {
        return await API.fetch('/admin/templates/pending');
    },

    async approveTemplate(templateId) {
        return await API.fetch(`/admin/templates/${templateId}/approve`, {
            method: 'POST'
        });
    },

    async rejectTemplate(templateId, reason) {
        return await API.fetch(`/admin/templates/${templateId}/reject`, {
            method: 'POST',
            body: JSON.stringify({ reason: reason || '' })
        });
    },

    async getMyTemplateRequests() {
        return await API.fetch('/templates/my-requests');
    }
};