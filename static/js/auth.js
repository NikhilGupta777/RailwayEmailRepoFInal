...
const Auth = {
    ...
    async loadUserDataAndInitApp() {
        try {
            ...
            console.log('Initializing enhanced profile module...');
            if (typeof EnhancedProfile !== 'undefined' && EnhancedProfile.init) {
                // Wait a bit for DOM to be fully ready
                setTimeout(() => {
                    EnhancedProfile.init();

                    // Profile display will be updated by EnhancedProfile module

                    // Double-check if profile button is working
                    setTimeout(() => {
                        const profileBtn = document.getElementById('profile-dropdown-toggle');
                        if (profileBtn && !EnhancedProfile.dropdownEventListenersAdded) {
                            console.warn('Profile button found but events not bound, forcing reinit...');
                            EnhancedProfile.forceInit();
                        }
                    }, 500);
                }, 100);
            } else {
                console.error('EnhancedProfile module not available or missing init method');
            }
