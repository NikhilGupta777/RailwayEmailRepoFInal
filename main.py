...
async def send_email(email_request: EmailRequest, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_user)):
    ...
    try:
        logger.info(f"SendGrid API key loaded: {SENDGRID_API_KEY[:10]}..." if SENDGRID_API_KEY else "None")
        sg = sendgrid.SendGridAPIClient(api_key=SENDGRID_API_KEY)
        ...
        response = sg.send(message)
        logger.info(f"SendGrid response status: {response.status_code}, body: {response.body}")
...
@app.get("/templates/my-requests")
def get_my_template_requests(db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_user)):
    templates = db.query(Template).filter(Template.created_by == current_user.id).all()
    return [{
        "id": t.id,
        "name": t.name,
        "subject": t.subject,
...
@app.post("/templates/request", response_model=TemplateSchema)
def request_template(template: AdminTemplateCreate, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_user)):
    import uuid
    template_id = str(uuid.uuid4())[:8]
    variables_json = json.dumps(template.template_variables) if template.template_variables else None
...
def get_pending_templates(db: Session = Depends(get_db), admin: DBUser = Depends(get_current_admin_user)):
    templates = db.query(Template).filter(Template.status == "pending").all()
    return [{
        "id": t.id,
        "name": t.name,
        "subject": t.subject,
        ...
        "sendgrid_template_id": t.sendgrid_template_id,
        "category": t.category,
...
def approve_template(template_id: str, db: Session = Depends(get_db), admin: DBUser = Depends(get_current_admin_user)):
    template = db.query(Template).filter(Template.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="Template not found")
...
def reject_template(template_id: str, data: dict, db: Session = Depends(get_db), admin: DBUser = Depends(get_current_admin_user)):
    template = db.query(Template).filter(Template.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="Template not found")
    template.status = "rejected"
    template.rejection_reason = data.get("reason", "")
    db.commit()
